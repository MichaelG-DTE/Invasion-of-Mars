...
func handle_step_climbing():
	step_status = "No vertical collision found"
	for i in player.get_slide_collision_count():
		var collision = player.get_slide_collision(i)
		if _is_vertical_surface(collision):
			#add step height measurement here
			var measured_height = _measure_step_height(collision) 
			if measured_height > MIN_STEP_HEIGHT and measured_height <= step_height and _is_valid_step_direction(collision): # checks whether the height of the step is greater than the step height var
				player.global_position.y += measured_height
				player.velocity = player.previous_velocity # keeps player velocity from prior frame, prevents velocity stopping
				player.camera.smooth_step(measured_height) # calls smooth step function
...
func _check_collision_surface(collision: KinematicCollision3D) -> bool:
	...
	var player_feet = _get_player_feet_position() # gets the lowest point of the player collision capsule
	collision_point.y = player_feet.y
	
	# uses a raycast to check whether the player is actually colliding with the step
...
func _measure_step_height(collision: KinematicCollision3D) -> float:
	...
	#starting and ending point of the ray to find the height of the step
...
func _is_valid_step_direction(collision: KinematicCollision3D) -> bool:
	var collision_normtal = collision.get_normal()
	var input_dir = player.get_input_direction()
	var movement_direction = player.transform.basis * Vector3(input_dir.x, 0, input_dir.y)
	if movement_direction.length() > MIN_MOVEMENT_LENGTH: # only climbs if the player is moving
		movement_direction = movement_direction.normalized()
		var dot_product = movement_direction.dot(-collision_normtal)
